...
public class Main {
    public static void main(String [] args){
        ...
        if (programaAplicado == null){
            ...
            JOptionPane.showMessageDialog(null, "Aplica para el programa "+ programaAplicado.getNombre()+ " con un costo de "+programaAplicado.getValorBase());
            int respuestaAceptaPrograma = JOptionPane.showConfirmDialog(null,"Desea matricular a "+ nombre+ " en el programa de "+programaAplicado.getNombre()+"?" , "Acepta programa", JOptionPane.YES_NO_OPTION);
            
            if(respuestaAceptaPrograma == JOptionPane.YES_OPTION){
                int respuestaAceptaEspeciales = JOptionPane.showConfirmDialog(null, "Quiere adicionar Materiales premium con un costo adicional de " + Programa.VALOR_MAERIALES_ESPECIALES + "?", "Materiales premium", JOptionPane.YES_NO_OPTION);
                if(respuestaAceptaEspeciales == JOptionPane.YES_OPTION){
                    valorAdicionales += Programa.VALOR_MAERIALES_ESPECIALES;
                }
                int respuestaAceptaTalleres = JOptionPane.showConfirmDialog(null, "Quiere adicionar Talleres de fin de semana con un costo adicional de " + Programa.VALOR_TALLERES_FIN_DE_SEMANA + "?", "Talleres de fin de semana", JOptionPane.YES_NO_OPTION);
                if(respuestaAceptaTalleres == JOptionPane.YES_OPTION){
                    valorAdicionales += Programa.VALOR_TALLERES_FIN_DE_SEMANA;
                }
                int respuetaAceptaIntensivas = JOptionPane.showConfirmDialog(null, "Quiere adicionar Clases intensivas con un costo adicional de " + Programa.VALOR_CLASES_INTENSIVAS + "?", "Clases intensivas", JOptionPane.YES_NO_OPTION);
